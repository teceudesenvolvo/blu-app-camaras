...
const CheckboxRow = styled.TouchableOpacity`flex-direction: row; align-items: center; margin-top: 18px; padding: 4px 0;`;
const Checkbox = styled.View`width: 24px; height: 24px; margin-right: 10px; border-radius: 6px; align-items: center; justify-content: center; background-color: ${({ checked, theme }) => checked ? theme.portal.primary : 'transparent'}; border-width: 2px; border-color: ${({ checked, theme }) => checked ? theme.portal.primary : theme.portal.border};`;
const CheckboxMark = styled.Text`color: #fff; font-size: 16px; font-weight: 900;`;
...
export default function AdminRecepcaoScreen({ navigation }) {
  ...
  const advanceStep = () => {
    if (flowStep === 2 && !requestForm.nome.trim()) { setError('Informe o nome do usuário para continuar.'); return; }
    setError('');
    setFlowStep(step => step === 0 ? 2 : Math.min(step + 1, 6));
  };
  ...
  return <PortalBackground>
    ...
    <Content contentContainerStyle={{ paddingBottom: 34 }}>
      ...
      {tab === 'novo' ? <FormCard><Title>Nova solicitação / encaixe</Title><Step>Etapa {flowStep + 1} de 7 · {['Setor', 'Atendimento', 'Usuário', 'Solicitação', 'Anexos', 'Protocolos e fila', 'Início'][flowStep]}</Step>
        ...
        {flowStep === 5 ? <><Text>Defina como o atendimento deve entrar na recepção.</Text><Choice active={priority} onPress={() => setPriority(true)}><ButtonText>Atendimento prioritário</ButtonText></Choice><Choice active={!priority} onPress={() => setPriority(false)}><ButtonText>Atendimento normal</ButtonText></Choice><CheckboxRow onPress={() => setShouldQueue(value => !value)}><Checkbox checked={shouldQueue}><CheckboxMark>{shouldQueue ? '✓' : ''}</CheckboxMark></Checkbox><Text style={{ marginTop: 0 }}>Adicionar como encaixe na fila</Text></CheckboxRow><Text>{requestForm.setor} · {requestForm.assunto || 'Tipo de atendimento não selecionado'} · {requestForm.nome}</Text><Button onPress={finishReceptionRequest} disabled={creating}><ButtonText>{shouldQueue ? 'Gerar protocolo e entrar na fila' : 'Gerar protocolo'}</ButtonText></Button></> : null}
